Function Track-Mouse {
    ...
    <Window 
        ...
        Width = "80" Height = "100" ShowInTaskbar = "False" ResizeMode = "NoResize"
        Topmost = "True" WindowStyle = "None" AllowsTransparency="True" Background="Transparent">  
        <Border BorderBrush="{x:Null}" BorderThickness="1" Height="100" Width="80" HorizontalAlignment="Left" 
        ...
        <Border.Background>
            ...
            <Grid ShowGridLines='False'>
                ...
                HorizontalContentAlignment="Center" />
                <Label x:Name='Left_Click' Grid.Column = '0' Grid.Row = '2' FontWeight = 'Bold'
                HorizontalContentAlignment="Center" />
                <Label x:Name='Right_Click' Grid.Column = '1' Grid.Row = '2' FontWeight = 'Bold'
                HorizontalContentAlignment="Center" />
                <Label x:Name='Mouse_Click' Grid.Column = '0' Grid.Row = '3' FontWeight = 'Bold'
                HorizontalContentAlignment="Center" />
            </Grid>
        </Border>
    </Window>
        ...
        $Window.Add_SourceInitialized({
            ...
            $timer.Add_Tick({
                ...
                <#if ([System.Windows.Forms.UserControl]::MouseButtons -eq "Left") {
                    if ($record -ne $null) {
                        if ($record.XPos -ne $mouse.X -or $record.YPos -ne $mouse.Y) {
                            $record = New-Object System.Object
                            $record | Add-Member -Type NoteProperty -Name XPos -Value $mouse.X
                            $record | Add-Member -Type NoteProperty -Name YPos -Value $mouse.Y
                            $record | Add-Member -Type NoteProperty -Name Time -Value $(Get-Date)
                            $record | Export-Csv -Path 'C:\users\tasul\documents\scripts\mousetrack.csv' -Append -NoTypeInformation
                        }
                    } else {
                        $record = New-Object System.Object
                        $record | Add-Member -Type NoteProperty -Name XPos -Value $mouse.X
                ...
                if ([System.Windows.Forms.UserControl]::MouseButtons -match "Left") {
                    $Left_Click.Content = "LC"
                } else {
                    $Left_Click.Content = ""
                }
                if ([System.Windows.Forms.UserControl]::MouseButtons -match "Right") {
                    $Right_Click.Content = "RC"
                } else {
                    $Right_Click.Content = ""
                }
                $Mouse_Click.Content = $MouseClick#>
            })
            #Start timer
